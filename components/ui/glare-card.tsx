...
export const GlareCard = ({
  children,
  className,
  coverImage, // tambahkan prop baru
}: {
...
interface GlareCardProps {
  children?: React.ReactNode  // sekarang optional
  className?: string
  coverImage?: string
}
  ...
  return (
    <div
      ...
      onPointerEnter={() => {
        ...
        setTimeout(() => {
          if (isPointerInside.current && refElement.current) refElement.current.style.setProperty("--duration", "0s");
        }, 300);
      }}
      onPointerLeave={() => {
          ...
          <div className={cn("h-full w-full bg-slate-950 relative", className)}>
            ...
            {coverImage && (
              <img
                src={coverImage}
                alt="cover"
                className="absolute inset-0 opacity-40 h-full w-full object-cover rounded-lg pointer-events-none"
              />
            )}
          </div>
        </div>
